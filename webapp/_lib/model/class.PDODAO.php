...
/**
 *
 * SocialCalc/webapp/_lib/model/class.PDODAO.php
 *
 * PDO DAO
    ...
    protected final function execute($sql, $binds = array()) {
        ...
        try {
            ...
            if ($config->getValue('debug')) {
                $exception_details .= '<br>SocialCalc could not execute the following query:<br> '.
                str_replace(chr(10), "", $stmt->queryString) . '  <br>PDOException: '. $e->getMessage();
            } else {
                $exception_details .=
                '<br>To see the technical details of what went wrong, set debug = true in SocialCalc\'s config file.';
            }
